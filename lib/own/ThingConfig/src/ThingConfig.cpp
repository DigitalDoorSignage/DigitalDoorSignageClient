...
void ThingConfigClass::getConfigJsonWithoutCredentials(char *buffer, size_t size)
{
	readConfig();
	if (_jsonConfig->containsKey("password"))
	{
		Logger.info("ThingConfig; getConfigJsonWithoutCredentials(), Password: ", _jsonConfig->get<char *>("password"));
		_jsonConfig->remove("password");
	}
	_jsonConfig->printTo(buffer, size);
	readConfig();
}
